#!/usr/bin/env python
#this script was modified by wangdingjie (wangdingjie@mail.kib.ac.cn)
...
def bad_nodes_txt(t,bad_nodes,png_name):
    gene_name = "_".join(sys.argv[1].split('.')[1].split("_")[:])
    f = open(png_name+'.txt','w')
    for n in bad_nodes:
        if n.is_leaf():
            f.write(n.name + '\n')
        else:
...
def main():
    ...
    if len(bad_nodes) > 0:
        bad_nodes_txt(t,bad_nodes,png_name)
